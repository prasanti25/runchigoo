...
export default function Tracking() {
  ...
  useEffect(() => {
    ...
    apiRequest(path, { token })
      .then((data) => setOrder(id ? data : (data.results || data)[0] || null))
      .catch((requestError) => setError(requestError.message))
      .finally(() => setLoading(false));
  }, [id, token]);
  const currentStage = useMemo(() => stages.indexOf(order?.status), [order]);
  if (loading) return <><Navbar /><main className="min-h-screen bg-[#fffaf7] p-10 text-center"><p>Loading your order…</p></main></>;
  if (error) return <><Navbar /><main className="min-h-screen bg-[#fffaf7] p-10 text-center text-red-600"><p>{error}</p></main></>;
  if (!order) return <><Navbar /><main className="min-h-screen bg-[#fffaf7] p-10 text-center"><p>No order was found.</p><Link to="/orders" className="mt-5 inline-flex rounded-xl bg-orange-500 px-5 py-3 font-semibold text-white">View orders</Link></main></>;
